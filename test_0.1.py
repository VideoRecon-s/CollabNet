...
def main():
    ...
    pth3_dir = "./CheckPoint/CollabNet_010.pth"
    checkpoint = torch.load(pth3_dir)
    ...
    with torch.no_grad():
        ...
        for i, cur_video_dir in enumerate(video_dir):
            ...
            output_data = 'CS ratio is {}, Avg PSNR/SSIM for {} is {:.2f}/{:.4f}' \
                .format(args.nonkey_sr, cur_video_dir, result_ave[0, i], result_ave[1, i])
            print(output_data)
    ...
    output_data = 'CS ratio is {}, Avg PSNR/SSIM for REDS is {:.2f}/{:.4f}' \
                .format(args.nonkey_sr, result_ave[0, len(video_dir)], result_ave[1, len(video_dir)])
    print(output_data)
